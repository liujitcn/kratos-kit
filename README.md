...
- `database/gorm`：GORM 客户端封装（含多数据库 driver 子模块，支持自动迁移后的表/字段注释回填；审计字段回填会跳过应用启动生命周期上下文）
...
其中 `queue` 模块内的 Redis Stream 实现默认会为生产者开启基于 `MAXLEN` 的长度裁剪；消费者在消费成功后会执行 `XACK` 并立即 `XDEL` 删除消息实体，保持“消费即删除”的队列语义。消费者执行 `Shutdown` 时会停止继续拉取新消息，并尽量处理完本地已拉取但尚未确认的消息后再退出。

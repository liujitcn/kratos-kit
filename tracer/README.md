...
- Exporter 注册表（Registry）
  ...
  - Factory 签名为：`func(ctx context.Context, cfg *configv1.Tracer) (traceSdk.SpanExporter, error)`，factory 可读取配置字段（endpoint、insecure、headers、tls 等）。
...
func init() {
    tracer.RegisterExporter("my-exporter", func(ctx context.Context, cfg *configv1.Tracer) (traceSdk.SpanExporter, error) {
...
- `NewTracerExporter(ctx context.Context, exporterName string, cfg *configv1.Tracer) (traceSdk.SpanExporter, error)` — 通过 registry 构建 exporter。  
- `NewTracerProviderWithShutdown(ctx context.Context, cfg *configv1.Tracer, appInfo *configv1.AppInfo) (traceSdk.TracerProvider, func(context.Context) error, error)` — 创建 provider 并返回 shutdown 函数。  
- `NewTracerProvider(ctx context.Context, cfg *configv1.Tracer, appInfo *configv1.AppInfo) error` — 兼容签名（仅返回 error）。  

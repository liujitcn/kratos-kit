...
`sdk.Runtime` 中已设置的实例。根密钥文件的首次生成和后续复用由本地 file provider 初始化流程负责，配置包本身不轮换根密钥。
...
应用通过 `LoadBootstrapConfig` 启动时，会在加载当前环境的本地配置文件前自动处理其中的 `ENC(...)` 标记，使用同一根密钥派生的配置密钥写回为 `ENC[...]`，然后再继续配置加载。已经是 `ENC[...]` 的值不会重复处理，未包含标记的文件不会回写；首次启动因此要求配置目录可写。生产环境可以在部署前离线完成加密，并将配置目录设为只读。

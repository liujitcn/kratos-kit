...
func (ht *HighPrecisionTimer) Stop() {
	...
	// 停止后清空任务索引和堆，避免并发 Add/Remove 后遗留的待执行任务影响后续状态判断。
	ht.heap = make(timerHeap, 0)
	ht.tasks = make(map[TimerTaskID]*TimerTask)
	heap.Init(&ht.heap)
	ht.mu.Unlock()
...
func (ht *HighPrecisionTimer) AddTask(task *TimerTask) TimerTaskID {
	...
	// 校验入参
	if task == nil {
		log.Warn("添加任务失败：任务为空")
		return ""
	}
	if !ht.running || task.ID == "" {
		log.Warn("添加任务失败：引擎未运行或任务ID为空", "task_id", task.ID)
		return ""
	}
	...
	if task.At.IsZero() && task.Cron != "" {
		if expr, err := cronexpr.Parse(task.Cron); err == nil {
			...
			log.Warn("解析Cron失败", "task_id", task.ID, "error", err)
			return ""
		}
	}
	...
	// 过滤At为零值的无效任务
	if task.At.IsZero() {
		log.Warn("任务At时间为零值", "task_id", task.ID)
		return ""
	}

	// 避免重复添加
	if _, exists := ht.tasks[task.ID]; exists {
		log.Warn("任务已存在", "task_id", task.ID)
		return ""
	}
...
func (ht *HighPrecisionTimer) RemoveTask(taskID TimerTaskID) bool {
	...
	if !ok || !ht.running {
		log.Warn("删除任务失败：任务不存在/引擎未运行", "task_id", taskID)
		return false
	}
	...
	log.Debug("任务删除成功", "task_id", taskID)
...
func (ht *HighPrecisionTimer) OnTimerTrigger(task *TimerTask) {
	if task != nil && task.Callback != nil {
		...
		if err != nil {
			log.Warn("定时任务回调执行失败", "task_id", task.ID, "error", err)
		}
	}
}

...
`NewHandler` 只初始化 SSE 处理器，不会调用 `net.Listen`，适合复用 Kratos HTTP Server 或原生 `net/http` 的现有端口。

### 自定义 stream 解析

默认情况下，SSE 服务端会从 query 参数中读取 `stream`，例如 `/events?stream=test`。如果业务希望从路径参数、自定义 Header 或其他位置读取 stream，可以通过 `WithStreamIDResolver` 覆盖默认行为。

例如，读取 `/events/{stream}` 形式的路径参数：

```go
srv := sse.NewHandler(
    sse.WithPath("/events/{stream}"),
    sse.WithStreamIDResolver(sse.ResolveStreamIDFromPathPrefix("/events/")),
    sse.WithAutoStream(true),
)
```

如果不传 `WithStreamIDResolver`，服务端仍默认兼容 `?stream=` 的 query 读取方式。

如果业务层已经完成鉴权或 stream 映射，也可以直接调用 `ServeStreamHTTP` 输出指定 stream：

```go
handler := sse.NewHandler(sse.WithAutoStream(true))

http.HandleFunc("/events/{stream}", func(w http.ResponseWriter, r *http.Request) {
    handler.ServeStreamHTTP(w, r, "actual-stream-id")
})
```

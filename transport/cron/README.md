...
- 支持启动前和运行时动态添加、移除和统计任务
- 并发安全，可在多协程中调用任务管理方法
- 停止服务时等待已开始执行的任务完成，默认最多等待 10 秒
- 支持时区、自定义 robfig/cron logger 和任务 panic 恢复
- 使用 `cron://scheduler` 虚拟注册端点，不额外占用监听端口
...
| `GetEntries() []cron.Entry` | 获取调度器中的任务快照 |
| `Scheduler() *cron.Cron` | 获取底层调度器 |
...
func main() {
	...
	entryID, err := srv.NewTimerJob("*/10 * * * * *", func() {
...
推荐在 `app.Run()` 前使用 `NewTimerJob` 注册任务。`StartTimerJob` 作为旧兼容入口，仍要求服务已经启动。
...
### 1. 启动前注册使用 `NewTimerJob`

`NewTimerJob` 可在启动前和启动后调用。旧入口 `StartTimerJob` 在服务尚未启动时仍返回错误：
...
`WithGracefullyShutdown(true)` 会等待已开始的任务完成，最长等待调用方 context 或 10 秒；
设置为 `false` 时只停止后续调度，不等待正在执行的任务。

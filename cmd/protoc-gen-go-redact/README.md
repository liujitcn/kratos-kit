...
`redact/v1` Proto 契约与本命令位于同一个独立 Buf 模块，发布地址为 `buf.build/liujitcn/kratos-kit-redact`。需要使用脱敏注解的项目，在自己的 `buf.yaml` 中添加该模块依赖：
...
- 如果你注册服务时未传入 bypass，或传入 `nil`，生成代码会使用 `redact.Falsy`（始终返回 `false`），因此内部方法会全部被拒绝。
...
可复用模板见 [templates/custom-template.tmpl](templates/custom-template.tmpl)。
...
定义代码生成插件的配置。当前配置仅针对 `redact/v1/redact.proto` 生成 Go protobuf、gRPC 和脱敏代码：
...
配置完成后，运行 `buf generate` 即可一次性生成 `redact/v1` 的 protobuf、gRPC 和脱敏代码；生成的脱敏代码依赖 `github.com/liujitcn/kratos-kit/redact` 运行时模块。执行 `make buf-push` 可将当前命令目录的 Proto 模块推送到 `buf.build/liujitcn/kratos-kit-redact`。

### .bufignore - 忽略文件

示例和测试数据位于 Buf 模块根 `redact` 之外，不会被发布；`.bufignore` 保留为本地工作区的额外忽略配置：

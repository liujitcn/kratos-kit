protoc-gen-go-redact (PGR)
=======================

[中文](README.md) | **[English](README_EN.md)** | [日本語](README_JA.md)

[![Go Report Card](https://goreportcard.com/badge/github.com/liujitcn/kratos-kit/cmd/protoc-gen-go-redact?dropcache)](https://goreportcard.com/report/github.com/liujitcn/kratos-kit/cmd/protoc-gen-go-redact)
[![Go Reference](https://pkg.go.dev/badge/github.com/liujitcn/kratos-kit/cmd/protoc-gen-go-redact.svg)](https://pkg.go.dev/github.com/liujitcn/kratos-kit/cmd/protoc-gen-go-redact)
[![License](https://img.shields.io/badge/license-apache2-mildgreen.svg)](./LICENSE)
[![GitHub release](https://img.shields.io/github/release/liujitcn/kratos-kit.svg)](https://github.com/liujitcn/kratos-kit/releases)
...
option go_package = "example.com/user/pb;user";
...
The `redact/v1` Proto contract is published with this command as an independent Buf module at `buf.build/liujitcn/kratos-kit-redact`. Add it to a consuming project's `buf.yaml`:

```yaml
deps:
  - buf.build/liujitcn/kratos-kit-redact
```
...
case *OneofMessage_Phone:
    v.Phone = redact.Mask(v.Phone, 3, 4, "*")
}
```
...
```go
import "github.com/liujitcn/kratos-kit/redact"
...
- If you omit bypass during registration, or pass `nil`, generated code falls back to `redact.Falsy` (always `false`), so internal RPCs are always blocked.
...
// Pass the optional bypass only when custom internal-call handling is needed.
pb.RegisterRedactedMyServiceServerWithBypass(s, impl,
...
See [templates/custom-template.tmpl](templates/custom-template.tmpl) for a reusable template starting point.
...
modules:
  - path: redact
    name: buf.build/liujitcn/kratos-kit-redact
    lint:
      use:
        - STANDARD
    breaking:
...
Defines the code generation plugin configuration. The current config generates Go protobuf, gRPC, and redaction code only for `redact/v1/redact.proto`:
...
plugins:
  ...
  - local: protoc-gen-go-redact
    out: redact
    opt:
      - paths=source_relative
```

Once configured, run `buf generate` to generate protobuf, gRPC, and redaction code in one step. Run `make buf-push` to publish the Proto module from this command directory to `buf.build/liujitcn/kratos-kit-redact`.

### .bufignore - Ignore File

Examples and test data live outside the Buf module root `redact` and are not published; `.bufignore` remains as an additional local workspace filter:

```
# Test data can have non-standard formatting
testdata
```

...
`redact/v1` Proto 契約はこのコマンドと同じ独立 Buf モジュール `buf.build/liujitcn/kratos-kit-redact` として公開します。利用するプロジェクトの `buf.yaml` に依存を追加してください：
...
- 登録時に bypass を省略するか `nil` を渡すと、生成コードは `redact.Falsy`（常に `false`）を使うため、内部 RPC は常に拒否されます。
...
// カスタム内部呼び出し処理が必要な場合のみ、オプションの bypass を渡します。
...
再利用可能なテンプレートは [templates/custom-template.tmpl](templates/custom-template.tmpl) を参照してください。
...
コード生成プラグインの設定を定義します。現在の設定は `redact/v1/redact.proto` に対して Go protobuf、gRPC、マスキングコードを生成します：
...
設定後、`buf generate` を実行するだけで、protobuf、gRPC、マスキングコードを一度に生成できます。`make buf-push` を実行すると、このコマンドディレクトリの Proto モジュールを `buf.build/liujitcn/kratos-kit-redact` に公開できます。

### .bufignore - 除外ファイル

サンプルとテストデータは Buf モジュールルート `redact` の外にあり公開対象ではありません。`.bufignore` はローカル作業領域の追加フィルターとして残しています：

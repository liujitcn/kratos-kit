...
// processFields extracts each field's redaction information.
func (m *Module) processFields(
	...
	// If this field uses regex masking, extract the pattern literal for the runtime call.
	if flData.IsRegex {
		var pattern string
...
// hashFuncName 将算法名称映射为运行时哈希函数名称。
func hashFuncName(algo string) string {
	switch algo {
	case "md5":
		return "redact.HashMD5"
	case "sha1":
		return "redact.HashSHA1"
	case "sha256":
		return "redact.HashSHA256"
	}
	return ""
}

...
func (c *Consumer) RegisterWithLastID(stream string, id string, fn ConsumerFunc) {
	c.RegisterWithLastIDContext(context.Background(), stream, id, fn)
}
...
func (c *Consumer) createConsumerGroup(ctx context.Context, stream string, id string) error {
	err := c.redis.XGroupCreateMkStream(ctx, stream, c.options.GroupName, id).Err()

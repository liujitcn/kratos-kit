...
func (p *Producer) Enqueue(msg *Message) error {
	return p.EnqueueContext(context.Background(), msg)
}

// EnqueueContext 使用指定上下文将消息追加到指定 Stream。
// 除非业务明确需要自定义消息 ID，否则建议交由 Redis 自动生成。
// 若由 Redis 自动生成，生成后的 ID 会回写到 msg.ID。
func (p *Producer) EnqueueContext(ctx context.Context, msg *Message) error {

...
func bootstrap(ctx *Context, initApp InitAppFunc) error {
	// 启动参数优先覆盖调用方传入的 app-info，再补齐默认值。
	ctx.InitAppInfo(flags.Project, flags.AppID, flags.InstanceID, flags.Name, flags.Version)
	...
	// 将全局日志器切换为当前应用 logger，确保包级别日志与 Kratos server 日志格式一致。

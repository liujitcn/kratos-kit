...
func init() {
	_ = logger.Register(logger.Zap, func(cfg *configv1.Logger) (*slog.Logger, error) {
...
func NewLogger(cfg *configv1.Logger) (*slog.Logger, error) {
	...
	var fileCore = zapcore.NewCore(fileEncoder, zapcore.NewMultiWriteSyncer(writeSyncer), lvl)
	coreArr = append(coreArr, newANSICleanCore(fileCore))
	...
	return logger.NewLegacyLogger(NewZapLogger(zapcore.NewTee(coreArr...))), nil
}

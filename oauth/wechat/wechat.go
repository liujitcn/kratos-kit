...
var wechatDefaultScopes = []string{"snsapi_login"}

// Provider 实现微信开放平台 OAuth 能力。
type Provider struct {
	conf *configv1.Provider
}

// New 创建微信开放平台 OAuth Provider。
...
// Name 返回微信开放平台 Provider 名称。
func (p *Provider) Name() provider.Type { return provider.Wechat }

// AuthURL 生成微信开放平台 OAuth 授权地址。
func (p *Provider) AuthURL(state string, opts ...provider.Option) string {
	...
	scopes := provider.ChooseScopes(p.conf.GetScopes(), o.Scopes, wechatDefaultScopes)
	params := url.Values{}
	...
	return provider.BuildAuthURL(wechatAuthURL, params) + "#wechat_redirect"
}

// GetToken 使用微信开放平台授权码换取 Token。
func (p *Provider) GetToken(ctx context.Context, code string, opts ...provider.Option) (*provider.Token, error) {
	o := provider.ApplyOptions(opts...)
	// 微信开放平台当前只支持授权码换取 Token。
	...
	if response.StatusCode < 200 || response.StatusCode >= 300 {
		return nil, fmt.Errorf("wechat token http status %d: %s", response.StatusCode, string(raw))
	}
...
func (p *Provider) GetUser(ctx context.Context, token *provider.Token) (*provider.User, error) {
	...
	raw := response.Body
	if response.StatusCode < 200 || response.StatusCode >= 300 {
		return nil, fmt.Errorf("wechat user http status %d: %s", response.StatusCode, string(raw))
	}
	err = json.Unmarshal(raw, &resp)
	if err != nil {
		return nil, err
	...
	if resp.ErrCode != 0 {
		return nil, fmt.Errorf("wechat userinfo error %d: %s", resp.ErrCode, resp.ErrMsg)
	}
